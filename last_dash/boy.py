...
def callback_handler(call):
    ...
    try:
        logging.info(f"press button : {call.data}")
        if call.data == "back_to_main":
            try:
                ...
                logging.error(f"ошибка при возвращение в меню{e}", exc_info=True)
        elif call.data == "m1a1":
            mode = 'text'
            bot.edit_message_text(
                chat_id=call.message.chat.id,
        ...
        elif call.data == "big_salary":
            bot.send_message(call.message.chat.id,
                            "если для вас важна высокая зарплата, то профессии в IT, принимательстве и инженерии могут быть подходящими вариантами.")
        elif call.data == "easy_learning":
            bot.send_message(call.message.chat.id,
                            "если вы ищете профессию с относительно легким обучением, то рассмотри те дизайнерские или журналистские специальности.")
        elif call.data == "remote_work":
            bot.send_message(call.message.chat.id,
                            "если для вас важна возможность удаленной работы, то профессии в IT, дизайне и журналистике могут быть хорошим выбором.")
        elif call.data == "creativity":
            bot.send_message(call.message.chat.id,
                            "если вам важна творческая составляющая, то профессии дизайнера, архитектора и журналиста могут подойти.")
        elif call.data == "demanded_profession":
            bot.send_message(call.message.chat.id,
                            "если для вас важна востребованность профессии, то IT, медицина и инженерия — это области с высоким спросом.")
        elif call.data == "humanities":
            bot.send_message(call.message.chat.id,
                            "если вам интересны гуманитарные специальности, то журналистика и преподавание могут быть подходящими вариантами.")
        elif call.data == "meticulous_work":
            bot.send_message(call.message.chat.id,
                            "если вам нравится кропотливая работа, то профессии ювелира и биотехнолога могут быть интересными вариантами.")
    except Exception as e:
        logging.error(f"ошибка в обработке кнопок {e} при нажатии {call.data}", exc_info=True)
